...
def main(config):
    ...
    if load == False:
        for frame_id, det in detections.items():
            ...
            if det.pts_obj_global.shape[0] > 200:
                ...
                obj.pts_obj_global = det.pts_obj_global
        ...
        for frame_id, obj in objects_recon.items():
            ...
            pts_canonical_space_homo = np.hstack((obj.pts_obj_global, np.ones((obj.pts_obj_global.shape[0], 1))))
            ...
            optimized_pcd[frame_id] = obj
            
        
        for frame_id, obj in optimized_pcd.items():
            if is_first_f:
                
                pts_canonical_space = obj.pts_canonical_space
                ...
                = np.concatenate((tmp, pts_canonical_space))
    ...
    for (frame_id, obj), (_, v) in zip(objects_recon.items(), velocity.items()):
        ...
        write_mesh_to_ply(mesh.vertices, mesh.faces, os.path.join(f"{save_dir}/{id}-accumulated-true", "%d.ply" % frame_id))
